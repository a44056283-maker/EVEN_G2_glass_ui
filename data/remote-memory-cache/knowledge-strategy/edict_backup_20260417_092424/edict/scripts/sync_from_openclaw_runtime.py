...
def main():
    ...
    try:
        ...
        for t in tasks:
            ...
            # 保留 Doing（<2min）、Blocked（报错）
            # Review 状态下如果包含"已完成"等关键词，也排除（说明工作已完成）
            state = t.get('state')
            now_text = t.get('now', '')
            done_keywords = ['已完成', '已生成', '已更新', '提案路径', '已写入', '已保存', '已汇报', '已完成']
            if state == 'Review' and any(kw in now_text for kw in done_keywords):
                continue  # 已完成，等待推进，不显示在看板
        ...
        # ── 只保留 JJC-* 旨意任务，丢弃所有 OC-* 会话任务 ──
        # OC任务来自各省部agent的临时session，容易造成看板混乱
        # 已完成的工作通过 JJC 正式任务流转，OC任务不应出现在旨意看板上
        tasks = [t for t in tasks if str(t.get('id', '')).startswith('JJC')]
