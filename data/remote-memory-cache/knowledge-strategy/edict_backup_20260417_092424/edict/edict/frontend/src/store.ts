...
import {
  ...
  type SubConfig,
  type ChangeLogEntry,
  type BotsBalancesResult,
  type BotsPositionsResult,
  type AgentReport,
} from './api';
...
export const TAB_DEFS: { key: TabKey; label: string; icon: string }[] = [
  ...
  { key: 'intervention', label: '⚡ 动态干预', icon: '⚡' },
...
interface AppStore {
  ...
  botsData: BotsBalancesResult | null;
  botsPositions: BotsPositionsResult | null;
...
export const useStore = create<AppStore>((set, get) => ({
  ...
  loadAgentReports: async () => {
    try {
      const reports = await api.agentReports();
      set({ agentReports: reports });
    } catch {
      // silently fail
    }
  },
  ...
  loadBots: async () => {
    try {
      ...
      set({ botsData: balances, botsPositions: positions });
    } catch {
      // silently fail
    }
  },
