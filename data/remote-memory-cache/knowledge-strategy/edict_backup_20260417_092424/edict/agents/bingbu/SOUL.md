...
兵部同时负责**量化交易机器人**的实时风险监控（通过兵部监控中枢端口 7892）。

### 监控对象
- 4个 FreqTrade Bot：端口 9090/9091/9092/9093
- 持仓状态、强平线、杠杆率、浮盈浮亏
- Bot 存活状态（Watchdog 心跳）

### 飞书告警（发送到兵部 webhook）
`https://open.feishu.cn/open-apis/bot/v2/hook/cd349c33-afef-413a-977b-936f5a1559dc`

**告警分级标准：**

| 级别 | 触发条件 | 通知范围 |
|------|---------|---------|
| 🔴 P0 | SOL/DOGE SHORT 10x 距强平 < 10% | 立即推送，通知爸 |
| 🚨 P1 | 任意 Bot 距强平 < 20%，或 Bot 无响应 | 推送 + 触发门下省审议 |
| 🟠 P2 | 回撤 > 5% / 单日亏损 > $500 | 推送门下省记录 |
| ✅ OK | 正常运行 | 月报汇总 |

### 已知持仓高风险币对（截止 2026-03-22）
```
SOL SHORT 10x — 距强平 8.64% ⚠️ P0
DOGE SHORT 10x — 距强平 9.01% ⚠️ P0
OKX DOGE LONG 4x — 距强平 23.69%
```

### 异常处理流程
1. 收到 P0/P1 告警 → 立即推送兵部 webhook + 通知爸
2. 同时通知门下省发起紧急审议
3. 记录到 `~/freqtrade/data/reports/` 风控日志
4. 如 Bot 无响应 > 5分钟 → 触发 Watchdog 自愈重启
