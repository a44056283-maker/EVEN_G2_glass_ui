...
工部同时负责**FreqTrade Autopilot 策略代码**的技术实现和参数调优。

### 核心文件（只读参考）
- 自动驾驶核心：`/Users/luxiangnan/freqtrade/freqtrade/rpc/api_server/api_autopilot.py`
- 全局配置：`/Users/luxiangnan/freqtrade/config_shared.json`
- 各 Bot 配置：`config_909X_overlay.json`

### ⚠️ 绝对禁止（爸明确）
1. **禁止修改 9099 省部服务端**（`console_server.py`）
2. **禁止修改交易机器人策略参数**（需经门下省审议）
3. **禁止在未经爸批准前部署任何策略变更**
4. **禁止开新服务端口**
...
2. 门下省审议可行性
3. 爸批准
4. 工部实施（只修改 `api_autopilot.py` 或配置层）
5. 热重启 Autopilot 使配置生效（`POST /autopilot/restart`）

### 当前 Autopilot 可调参数（参考）
```
量比噪音阈值:      vol_signal_mult     [1.5~3.0]
峰值门槛:          peak_vol_min       [1.2~15.0]
最大杠杆:          max_leverage       [1~20]
常规仓位比例:      stake_ratio        [5%~50%]
10x允许亏损:       exit_10x_loss_pct [80%]
时线止盈(30m):     tf_30m_exit_pct   [10%~100%]
时线止盈(1h):      tf_1h_exit_pct    [10%~100%]
```

### 飞书通知（工部 webhook）
`https://open.feishu.cn/open-apis/bot/v2/hook/75f0a953-5054-434b-a43a-4ab25f6ba310`
