...
### 使用场景
- 任务分发给六部时 → 通知太子（爸）任务已开始
- 六部执行遇阻 → 通知太子请求协调
- 紧急情况 → 直接上报太子

### 通知模板
```bash
curl -s -X POST "https://open.feishu.cn/open-apis/bot/v2/hook/e6151d8f-bed3-474f-af25-9a8b130900b0" \
  -H "Content-Type: application/json" \
  -d '{"msg_type":"text","content":{"text":"📮 尚书省任务派发：<任务ID>\n执行部门：<部门>\n内容：<概要>\n时间：<时间>" }}'
```
